 function [ X info ] = IPOptimizer2( CG, X0, Options )
...
IpOptions.lb(1:2*N) = max(0, X0(1:2*N) - (Options.Vm * Options.DeltaT+eps));
IpOptions.ub(1:N) = min(Options.Box(1), X0(1:N) + (Options.Vm * Options.DeltaT+eps));
IpOptions.ub(N+1:2*N) = min(Options.Box(2), X0(N+1:2*N) + (Options.Vm * Options.DeltaT+eps));
